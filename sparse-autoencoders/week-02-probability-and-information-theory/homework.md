...
Suppose you observe data $x\_1, x\_2, \ldots, x\_N$ drawn i.i.d. from $\mathcal{N}(\mu, \sigma^2)$.

**(a)** Write down the log-likelihood $\ell(\mu, \sigma^2) = \log \prod\_{i=1}^N p(x\_i; \mu, \sigma^2)$.
...
**(d)** The MLE for $\sigma^2$ is $\hat{\sigma}^2 = \frac{1}{N}\sum\_{i=1}^N (x\_i - \bar{x})^2$, which is the *biased* sample variance (divides by $N$ instead of $N-1$). Show that this estimator is biased by computing $\mathbb{E}[\hat{\sigma}^2]$ and showing it equals $\frac{N-1}{N}\sigma^2$, not $\sigma^2$.

*Hint for (d):* Use the identity $\sum\_i (x\_i - \bar{x})^2 = \sum\_i x\_i^2 - N\bar{x}^2$, then take expectations using $\mathbb{E}[x\_i^2] = \sigma^2 + \mu^2$ and $\mathbb{E}[\bar{x}^2] = \frac{\sigma^2}{N} + \mu^2$.
...
**(a)** Suppose we have a classification problem with $K$ classes. The true label for sample $i$ is a one-hot vector $\mathbf{y}\_i \in \lbrace 0, 1\rbrace ^K$ (with $y\_{ik} = 1$ for the true class $k$). Our model outputs a probability vector $\hat{\mathbf{y}}\_i = \text{softmax}(f\_\theta(\mathbf{x}\_i))$. Write down the negative log-likelihood of the data under this model.
...
**(c)** Compute the conditional entropy $H(Y|X) = \sum\_x p(x) H(Y|X=x)$. Verify that $H(X, Y) = H(X) + H(Y|X)$ (chain rule for entropy).
...
**(a)** Derive the KL divergence between two univariate Gaussians $p = \mathcal{N}(\mu\_1, \sigma\_1^2)$ and $q = \mathcal{N}(\mu\_2, \sigma\_2^2)$.
...
where $\pi\_k \geq 0$, $\sum\_k \pi\_k = 1$ are the mixing weights.
...
**E-step:** For each data point $x\_i$ and component $k$, compute the "responsibility":
$$
