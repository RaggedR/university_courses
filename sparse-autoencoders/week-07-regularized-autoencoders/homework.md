...
Prove that a linear autoencoder with encoder $f(x) = Wx + b$ (where $W \in \mathbb{R}^{d\_z \times d\_x}$) and decoder $g(z) = W'z + b'$ (where $W' \in \mathbb{R}^{d\_x \times d\_z}$) can represent the identity function whenever $d\_z \geq d\_x$.

Specifically, construct explicit matrices $W, W'$ and biases $b, b'$ such that $g(f(x)) = x$ for all $x \in \mathbb{R}^{d\_x}$.
...
with $W\_1 \in \mathbb{R}^{m \times d\_x}$, $W\_2 \in \mathbb{R}^{d\_z \times m}$, and the decoder is linear: $g(z) = W'z + b'$. Suppose $d\_z < d\_x$ but $m$ is very large (say, $m \gg d\_x$).
...
Train a linear overcomplete autoencoder ($d\_z = 1000$, $d\_x = 784$) on MNIST with no regularization. Report the training loss after convergence. Compute the product $W' W$ and compare it to the identity matrix $I\_{784}$. What do you observe?
...
1. Derive the expression for the Jacobian $J\_f(x)$ in terms of $W$ and the activations $f(x)$.
2. Show that $\Vert J\_f(x)\Vert \_F^2 = \sum\_{i=1}^{d\_z} [f\_i(x)(1 - f\_i(x))]^2 \Vert w\_i\Vert ^2$.
...
2. Compute the average $\Vert J\_f(x)\Vert \_F^2$ over the test set.
...
Train four autoencoders on MNIST, all with $d\_z = 128$:
...
Substitute your Taylor expansion into the reconstruction loss $\Vert x - r(\tilde{x})\Vert ^2$ and expand. You should get three terms.
...
- For any matrix $A$: $\mathbb{E}[\Vert A\epsilon\Vert ^2] = \sigma^2 \Vert A\Vert \_F^2$
...
Train two overcomplete autoencoders on MNIST with $d\_z = 1000$ (recall $d\_x = 784$):
...
For each model, compute the average activation of each latent unit over the test set: $\bar{a}\_i = \frac{1}{N} \sum\_{n=1}^{N} f\_i(x\_n)$.
