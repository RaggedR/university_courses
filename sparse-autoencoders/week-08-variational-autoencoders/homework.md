...
Write down the log marginal likelihood $\log p_\theta(x)$ as the log of an integral over the joint distribution $p_\theta(x, z)$. Then introduce the variational distribution $q_\phi(z|x)$ by multiplying and dividing inside the integral.
...
Use $p_\theta(x, z) = p_\theta(x|z) p(z)$ to decompose this into:
...
*Hint: Start from the definition of $D_{\text{KL}}(q_\phi(z|x) \| p_\theta(z|x))$ and use Bayes' rule to substitute for $p_\theta(z|x)$.*

### Part (e): Interpretation

1. Why does the gap being $D_{\text{KL}}(q_\phi(z|x) \| p_\theta(z|x)) \geq 0$ guarantee that the ELBO is indeed a lower bound?
...
Compute $D_{\text{KL}}$ for $q = \mathcal{N}(\mu, \sigma^2 I)$ with $d_z = 2$ for the following cases:
...
Starting from the definition of KL divergence as an expectation, derive the closed-form expression for the KL divergence $D_{\text{KL}}(q(z|x) \| p(z))$ where $q(z|x) = \mathcal{N}(\mu, \text{diag}(\sigma^2))$ and $p(z) = \mathcal{N}(0, I)$.
...
Prove that $D_{\text{KL}}(q \| p) = 0$ if and only if $\mu = 0$ and $\sigma^2 = \mathbf{1}$ (the vector of all ones).

*Hint: Show that for each dimension $j$, the function $f(\mu_j, \sigma_j^2) = \mu_j^2 + \sigma_j^2 - \log \sigma_j^2 - 1$ has a unique minimum at $\mu_j = 0, \sigma_j^2 = 1$.*
...
- **Encoder:** $784 \to 512 \to 256$, ReLU activations. The final layer branches into two heads:
  - $\mu$: Linear $256 \to d_z$
  - $\log \sigma^2$: Linear $256 \to d_z$
- **Decoder:** $d_z \to 256 \to 512 \to 784$, ReLU activations, sigmoid on the final layer.
- **Latent dimension:** $d_z = 20$ (but also experiment with $d_z = 2$ for visualization).
...
For the $d_z = 2$ model:
4. Plot the latent space: encode all test images, plot $\mu_1$ vs. $\mu_2$, colour-coded by digit label. How well-separated are the clusters?
...
Using your trained VAE from Problem 4 ($d_z = 20$):
...
1. Compute $z_{\text{smile}} = \bar{z}_3 - \bar{z}_2 + \bar{z}_5$ (where $\bar{z}_k$ is the average encoding of digit $k$). What digit does decoding this produce?
...
Select two test images of different digits (e.g., a "2" and a "7"). Encode both to get $\mu_1$ and $\mu_2$.

Create a sequence of 10 interpolated latent vectors:
$$z_t = (1-t)\mu_1 + t\mu_2, \quad t \in \{0, 0.11, 0.22, \ldots, 1.0\}$$

Decode each $z_t$ and display the sequence. The transition should be smooth.
...
Train VAEs with $\beta \in \{0.1, 0.5, 1.0, 2.0, 5.0, 10.0\}$, all with $d_z = 10$. For each:
...
2. For each latent dimension $j$, create a sequence where $z_j$ varies from $-3$ to $+3$ while all other dimensions stay fixed at $\mu$.
